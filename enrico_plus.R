...
modvolatilityarch <- garchFit(ritorni1 ~ arma(6, 1) + garch(9, 0),
    data = ritorni1,
    include.mean = FALSE, trace = FALSE
)

modvolatilitygarch <- garchFit(ritorni1 ~ arma(6, 1) + garch(1, 1),
    data = ritorni1,
    include.mean = FALSE, trace = FALSE
)
...
# Primo comando giusto
predict(modvolatilitygarch, n.ahead = 1)

# secondo comando giusto
...
fit <- ugarchfit(spec = spec, data = ritorni1)
forecast <- ugarchforecast(fit, n.ahead = 1)

fitnuovo <- ugarchfit(spec = spec, data = as.numeric(na.omit(ritorni1)))
forecastnuovo <- ugarchforecast(fit, n.ahead = 1)
...
fit_brent <- ugarchfit(spec = spec, data = ritorni1)
show(fit_brent)
# 3. Generazione delle previsioni
previsioni_garch <- ugarchforecast(fit_brent, n.ahead = 4)
# Per vedere i rendimenti attesi (Media)
print(fitted(previsioni_garch))

# Per vedere la volatilità attesa (Sigma) - La parte più importante!
print(sigma(previsioni_garch))

# backtesting(qualita del forecast)
library("MTS")

length(ritorni1)
# Definisci il backtest (rolling window)
# n.start è il tuo 'origin' (1013)
roll <- ugarchroll(spec,
    data = ritorni1, n.start = 1013,
    refit.window = "moving", refit.every = 10
)

# Calcola le statistiche di errore (RMSE, MAE)
report(roll, type = "fpm")
# forecast con materiale
ritorno_previsto <- predict(modvolatilitygarch, 4)
names(ritorno_previsto)


# parte diversa da forecasting
